...
module.exports = class AthenaExpress {
    ...
    /**
     * 
     * For queries run with waitForResults=false,
     * you can poll the status of the query here
     * 
     * Calling await .query() will block until the
     * query finishes running.
     * 
     * This is useful if, for example, you want to
     * show the user progress through the several
     * running query stages
     * 
     * @param {*} queryId Result of calling .query() with your SQL
     * @returns QUEUED | RUNNING | SUCCEEDED | FAILED | CANCELLED
     */
    async checkQueryProgress(queryId) {
        const config = { ...this.config };
        config.QueryExecutionId = queryId
        let execution = await helpers.getQueryExecution(queryId)
